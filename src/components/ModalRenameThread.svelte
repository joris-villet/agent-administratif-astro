<script lang="ts">
  ...
  let { threadId, currentTitle, onclose }: {
    threadId: string;
  ...
  async function confirm() {
    ...
    error = null;
    try {
      await ky.post(`${import.meta.env.PUBLIC_FASTIFY_URL}/api/thread/rename`, {
        credentials: 'include',
        json: { threadId, title: newTitle.trim() },
      });
      const thread = storeThreads.threads.find((t: IThread) => t.threadId === threadId);

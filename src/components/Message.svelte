...
{#if role === "user"}
  <div class="flex justify-end mb-6">
    <div
      class="max-w-[75%] bg-message-user text-white px-4 py-2.5 rounded-2xl rounded-br-sm shadow-sm"
    >
      <p class="anim-enter whitespace-pre-line font-mono leading-none">
        {content}
      </p>
    </div>
  </div>
...
{#if loading && isNewMessage}
  <div class="flex justify-start mb-6 items-center gap-2">
    <div
      class="rounded-full bg-slate-200 border border-blue-100 flex items-center justify-center shrink-0 mb-0.5"
    >

<script lang="ts">
  ...
  async function confirm() {
    ...
    try {
      await ky.delete(
        `${import.meta.env.PUBLIC_BACKEND_URL}/api/thread/delete`,
        {
          credentials: "include",
          json: { threadId },
        }
      );
      storeThreads.threads = storeThreads.threads.filter(
...
>
  ...
  >
    ...
    <p class="text-neutral-400 text-sm">
      Cette action est irréversible. La conversation sera définitivement
      supprimée.
    </p>
